...
import { BYOMAuthGuard } from '@/components/byom/byomauthguard';
...
export default function CustomizePage({ params }: { params: Promise<{ type: MerchType }> }) {
  ...
  const allStickers = [...stickers, ...uploadedStickers];
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      const newStickerId = `custom-${Date.now()}`;
      const newSticker = {
        id: newStickerId,
      ...
      setUploadedStickers([...uploadedStickers, newSticker]);
  ...
  const hasCustomStickers = () => {
    const allPlacedStickers = [
      ...customization.front.stickers,
      ...customization.back.stickers,
      ...customization.side.stickers,
    ];

    return allPlacedStickers.some(placedSticker => {
      const sticker = allStickers.find(s => s.id === placedSticker.stickerId);
      return sticker?.isCustom;
    });
  };
  ...
  const handleNext = () => {
    const customizationData = {
      ...
      requiresApproval: hasCustomStickers()
    };
                    ...
                    {uploadedStickers.length > 0 && (
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex gap-2">
                        ...
                        <p className="text-xs text-yellow-800">
                          Custom uploads require approval before your order can be processed.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-3 gap-3 max-h-96 overflow-y-auto">
                      {allStickers.map((sticker) => (
                        <button
                          ...
                          className={cn(
                            "aspect-square border-2 rounded-lg hover:border-grey transition-colors p-2 relative",
                            sticker.isCustom ? "border-yellow-300" : "border-accent-2"
                          )}
                        >
