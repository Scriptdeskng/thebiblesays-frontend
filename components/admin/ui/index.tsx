  ...
  return (
    <div className="bg-admin-primary/4 rounded-xl p-6 hover:shadow-md transition-shadow">
      ...
      <div className="mt-10 flex items-center gap-2">
        <div
          className={`flex items-center space-x-1 text-sm rounded-lg py-1 px-2 ${
            isPositive ? "text-[#2AA31F] bg-[#2AA31F]/15" : "text-red-600"
          }`}
        >
...
export function Badge({ children, variant = "default" }: BadgeProps) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 rounded-lg capitalize ${variants[variant]}`}
    >
      {children}
    </span>
  );
...
export function Input({ label, error, className = "", ...props }: InputProps) {
  return (
    <div>
      {label && (
        <label className="block text-admin-primary mb-1">{label}</label>
      )}
      <input
        className={`w-full px-4 py-2 border border-admin-primary/35 rounded-lg focus:outline-none transition-all ${
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 px-4">
      ...
      {description && (
        <p className="text-sm text-grey text-center mb-4">{description}</p>
      )}
